...
class AdminContent extends Admin{
	...
	public function index(){
        ...
        */
        ...
        $this->assign('categoryList',model('Category')->loadList());
        $this->assign('positionList',model('Position')->loadList());
    ...
    public function info(){
        ...
        if (input('post.')){
            ...
            if(input('post.remote') == 'on'){
                $_POST['content'] = $model->GetContFile(input('post.content'));
            }
            if ($content_id){
                $status=$model->edit();
            }else{
            ...
            $this->assign('category_list',model('Category')->loadList());
            $this->assign('position_list',model('Position')->loadList());
            $this->assign('tplList',model('admin/Config')->tplList());//模板文件
            $this->assign('info',$model->getInfo(input('id')));//页面信息
//            dump($model->getInfo(input('id')));die;
