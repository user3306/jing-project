...
//      根据userid查询项目经理名下的资产
    ...
    public function getcode($cli_openid,$type=2){
        ...
        $file = $qr_path.$filename;
        if(file_exists($file)){
            $wechatcode='https://'.$_SERVER['SERVER_NAME'].'/Uploads/'.$filename.'?ver=1';
        }else{
	...
	*/
	public function day_drainage(){
		$datas = $this->request->post();
        $cli_openid = isset($datas['cli_openid']) ? $datas['cli_openid'] : 'oaCz74tT9R4v0G_h7WlsbPCrsmmg';
        $token  = isset($datas['token']) ? $datas['token'] : md5($cli_openid.'JKSHFUHFF_FFF4s5f45sW'.date('Ymd'));
		...
		$userid = self::getUserid($cli_openid);
        $daynum = BindingModel::where('userid',$userid)->where($whereOr)->count('id');
		echo BindingModel::getLastSql();
        $data['daynum'] = $daynum;
		$day_drainage = Db::name('changcheng_bingdinginfo2019')
                            ->alias('b')
                            ->leftJoin('changcheng_user_information2019 u',' b.unionid=u.unionId')
                            ->leftJoin('changcheng_entryinfo2019 e ',' b.union=e.unionid')
                            ...
                            ->select();
		echo Db::name('changcheng_bingdinginfo2019')->getLastSql();
		die;
        }
