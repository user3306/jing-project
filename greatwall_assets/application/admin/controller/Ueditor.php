...
class Ueditor extends Admin
{
    ...
    public function imageUp(){
        ...
        if (!empty($_FILES[$file]['name'])){
            ...
            $info = $file->validate(['size'=>1024*2*1000,'ext'=>'gif,png,jpg,jpeg,bmp'])->move(ROOT_PATH . 'public' . DS . 'uploads'. DS . 'admin'. DS . 'goods');

            if($info){
                // 成功上传后 获取上传信息
                $data['url']='/uploads/admin/goods/'.str_replace('\\','/',$info->getSaveName());
                $data['original']='/uploads/admin/goods/'.str_replace('\\','/',$info->getSaveName());
                $data['ext']=$info->getExtension();
                $data['size']=$info->getSize();
                $data['create_time']=time();
                $rs=model('admin/File')->add($data);
                if ($rs>0){
                    $return_data['url'] = $data['url'];
                    $return_data['title'] = '';
                    $return_data['original'] = $data['url'];
