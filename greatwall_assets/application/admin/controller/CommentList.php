...
class CommentList extends Admin{
	...
	public function index(){
		//筛选条件
        $where = array();
        $cli_mobile = input('cli_mobile');  //员工手机号
        $pro_mobile = input('pro_mobile');  //项目经理手机号
        ...
        if ($userinfo['group_name'] != "超级管理员" && $userinfo['group_name'] != "管理层" &&  $userinfo['group_name'] != "部门负责人" ) {
           $where[] = ['a.userid','in',$userinfo['nicename']];
            ...
            $where[] = ['a.userid','in',$allInfo];
        }

        if(!empty($cli_mobile)){
            $where[] = ['c.cli_mobile','like','%'. $cli_mobile.'%'];
        }

        if(!empty($cli_mobile)){
            $where[] = ['a.asset_managerline','like','%'. $cli_mobile.'%'];
        }

        if (!empty($starttime)) {
            $where[] = ['a.asset_managerline','>=',strtotime($starttime.' 00:00:00')];
        }

        if (!empty($endtime)) {
            $where[] = ['a.asset_managerline','<=',strtotime($endtime.' 23:59:59')];
        }

        $list = model('Comment')->getCommentList($where,20);
