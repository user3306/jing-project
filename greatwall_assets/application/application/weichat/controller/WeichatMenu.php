...
class WeichatMenu extends Admin {
    ...
    public function createmenu()
    {
        ...
        foreach ($topMenu as $key => $vo) {
            ...
            $itemMenu = model('weichat_menu')->where([['parent_id','=',$vo['menu_id']]])->order('sort','asc')->limit(20)->select();
            ...
            if (count($itemMenu)) {
                ...
                }else if($vo['type']=='miniprogram'){
					$button[$key]['type'] = $vo['type'];
					$button[$key]['name'] = $vo['name'];
					$button[$key]['url'] = "http://mp.weixin.qq.com";
					$button[$key]['appid'] = $vo['appid'];
					$button[$key]['pagepath'] = $vo['pagepath'];
