...
class AssetType extends Admin{
	...
	public function index(){
        ...
        if(!empty($assettype)){
            $where[] = ['a.assettype|t.assettype','like','%'.$assettype.'%'];
        }
		
		$searchinfo  = array();
		$searchinfo = request()->param();

        $list = model('AssetType')->getAssetTypeList($where,20,$searchinfo);
        //位置导航
    ...
    public function del($id){
        ...
        $num = model('AssetType')->getAssetTypeTotal(array('pid'=>$id));
        if ($num > 0) {
            return ajaxReturn(0,'请先删除子集内容');
        }
